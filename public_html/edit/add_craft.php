...
$countmats = mysqli_num_rows($q_maters);
   ...
   for ($i = 1; $i < $countmats; $i++) {
   $arrmats = mysqli_fetch_assoc($q_maters);
   echo '<p><div><input type="hidden" name="item_id" value="'.$arrmats['item_id'].'" autocomplete="off" dislay="none"> <input type="text" name="item_name" size="30" value="'.$arrmats['item_name'].'" autocomplete="off" dislay="none"> <input type="number name="mater_need" size="5" value= "'.$arrmats['mater_need'].'"></div></p>';
